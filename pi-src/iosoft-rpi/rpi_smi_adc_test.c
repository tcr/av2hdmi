...
#define SMI_TIMING      SMI_TIMING_42M
...
int fn_setup(uint32_t nsamp)
{
    ...
    gpio_out(TEST_PIN, 0);
#endif
    map_uncached_mem(&vc_mem, VC_MEM_SIZE(((nsamp*SAMPLE_SIZE)*4)+RX_VMEM_OFFSET+(sizeof(DMA_CB)*6)));
    smi_dmc->dmaen = 1;
    smi_cs->enable = 1;
    smi_cs->clear = 1;
#endif

    adc_dma_start(&vc_mem, nsamp);
    smi_start(nsamp, 1);
}
...
void fn_collect(uint32_t nsamp, uint16_t* dest) {
    MEM_MAP *mp = &vc_mem;
    DMA_CB *cbs=mp->virt;
    uint32_t *data=(uint32_t *)(cbs+6), *rxdata=data+RX_VMEM_OFFSET, *rxdata2=rxdata+(nsamp/2);
    volatile uint32_t *id=data+0x20;
    ...
    // lock_vc_mem(mp->fd, mp->h);
    memcpy(dest, rxdata, nsamp*SAMPLE_SIZE);
    ...
    // uint16_t* src;
    // if (last_id == ID0) {
    //     src = rxdata + (nsamp + SAMPLE_SIZE);
    // } else {
    //     src = rxdata;
    // }

    // volatile int current_block = dma_conblock(DMA_CHAN_A);
    // int counter = 0;
    // while ((dma_conblock(DMA_CHAN_A) - MEM_BUS_ADDR(mp, &cbs[0])) <= 0x40) {
        // usleep(0);
        // counter += 1;
        // printf("%x\n", dma_conblock(DMA_CHAN_A) - MEM_BUS_ADDR(mp, &cbs[0]));
    // }
    // while (dma_active(DMA_CHAN_A)) {
    // printf("buffer_id: %d\n", *id);
    // adc_dma_end(rxbuff, target_buffer, nsamp);
    // printf("buffer_id: %d on %x\n", *id, dma_conblock(DMA_CHAN_A));
    // printf("%x\n", dma_conblock(DMA_CHAN_A) - MEM_BUS_ADDR(mp, &cbs[0]));
    // usleep(8000);
    // putchar('\n');
    // putchar('\n');
...
{
    DMA_CB *cbs=mp->virt;
    uint32_t *data=(uint32_t *)(cbs+6), *pindata=data+8, *modes=data+0x10;
    uint32_t *modep1=data+0x18, *modep2=modep1+1;
    uint32_t *id = data+0x20, *id0=id+1, *id1=id0+1;
    uint32_t *rxdata1=data+RX_VMEM_OFFSET;
    uint32_t *rxdata2=rxdata1+(nsamp * SAMPLE_SIZE);
    uint32_t *rxdata3=rxdata2+(nsamp * SAMPLE_SIZE);
    uint32_t i;
    ...
    *id1 = ID1;
    ...
    cbs[2].tfr_len = nsamp * SAMPLE_SIZE;
    cbs[2].srce_ad = REG_BUS_ADDR(smi_regs, SMI_D);
    cbs[2].dest_ad = MEM_BUS_ADDR(mp, rxdata1);
    // cbs[2].next_cb = MEM_BUS_ADDR(mp, &cbs[3]);
    // Control block 2: set buffer id
    // cbs[3].ti = (DMA_SMI_DREQ << 16) | DMA_WAIT_RESP;
    // cbs[3].tfr_len = 4;
    // cbs[3].srce_ad = MEM_BUS_ADDR(mp, id0);
    // cbs[3].dest_ad = MEM_BUS_ADDR(mp, id);
    // cbs[3].next_cb = MEM_BUS_ADDR(mp, &cbs[4]);
    // // Control block 2: read data
    // cbs[4].ti = (DMA_SMI_DREQ << 16) | DMA_CB_DEST_INC;
    // cbs[4].tfr_len = nsamp * SAMPLE_SIZE;
    // cbs[4].srce_ad = REG_BUS_ADDR(smi_regs, SMI_D);
    // cbs[4].dest_ad = MEM_BUS_ADDR(mp, rxdata2);
    // cbs[4].next_cb = MEM_BUS_ADDR(mp, &cbs[5]);
    // Control block 2: set buffer id
    // cbs[5].ti = (DMA_SMI_DREQ << 16) | DMA_WAIT_RESP;
    // cbs[5].tfr_len = 4;
    // cbs[5].srce_ad = MEM_BUS_ADDR(mp, id1);
    // cbs[5].dest_ad = MEM_BUS_ADDR(mp, id);
    // cbs[5].next_cb = MEM_BUS_ADDR(mp, &cbs[0]);
